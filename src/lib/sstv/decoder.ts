...
export class SstvDecoder {
  ...
  tick(tap: PcmTap): void {
    ...
    while (this.state.nextScanLine < Math.min(targetScanLine, scanLineCount)) {
      const scanLineStart = t0 + Math.round(this.state.nextScanLine * effectiveRowSamplesFloat)
      ...
      if (rawMs !== undefined && Number.isFinite(rawMs) && Math.abs(rawMs) <= SLANT_FILTER_MS) {
