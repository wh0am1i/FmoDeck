...
export function LogsTable({ onRowClick }: Props) {
  ...
  const { countByCall, todayCalls } = useMemo(() => {
    const today = startOfLocalToday(Date.now())
    const countByCall = new Map<string, number>()
    const todayCalls = new Set<string>()
    for (const r of all) {
      countByCall.set(r.toCallsign, (countByCall.get(r.toCallsign) ?? 0) + 1)
      if (r.timestamp >= today) todayCalls.add(r.toCallsign)
    }
    return { countByCall, todayCalls }
  }, [all])
      ...
      <table className="hud-mono w-full text-sm" aria-label="QSO 日志列表">
        ...
        <tbody>
          {slice.map((r) => {
            ...
            const calledToday = todayCalls.has(r.toCallsign)
            return (
              <tr
                key={r.logId}
              ...
              >
                ...
                <td className="px-3 py-2 text-primary">
                  <span className="flex flex-wrap items-center gap-1.5">
                    ...
                    {!isToday && calledToday && (
                      <span
                        className="rounded-sm border border-muted-foreground bg-muted/30 px-1 py-0 text-[10px] uppercase leading-4 text-muted-foreground"
                        title="此呼号今日也通联过"
                      >
                        · 今日也联
                      </span>
