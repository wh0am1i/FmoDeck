...
**FmoLogs 参考文件：**
- IndexedDB schema：`/Users/wh0am1i/FmoLogs/src/services/db.js` L287–428（LOGS_DB_NAME=`FmoLogsData`, per-callsign stores, dedupKey=`${utcDate}_${toCallsign}`, 索引 timestamp/toCallsign/toGrid/relayName/utcDate）
- sql.js loader：L1–14
...
export async function putItem<T>(db: IDBDatabase, store: string, value: T, key?: IDBValidKey): Promise<void> {
  const tx = db.transaction(store, 'readwrite')
  const req = key === undefined ? tx.objectStore(store).put(value) : tx.objectStore(store).put(value, key)
  await asPromise(req)
}

export async function getItem<T>(db: IDBDatabase, store: string, key: IDBValidKey): Promise<T | undefined> {
  ...
  it('支持 limit 和 offset', () => {
    insertRecords(
      ...
      Array.from({ length: 10 }, (_, i) =>
        makeQso({ fromCallsign: 'BA0AX', timestamp: i * 1000 })
      )
    )
...
export function getCallsignStats(
  ...
  try {
    ...
    stmt.step()
    const row = stmt.getAsObject() as { count: number; firstTime: number | null; lastTime: number | null }
    return {
      count: row.count ?? 0,
