...
const cases = [
  { fromCall: 'BA0AX', fromSsid: 5, type: 'CONTROL', action: 'NORMAL', timeSlot: 29334000, counter: 0, secret: 'ABCDEFGHJKLM' },
  { fromCall: 'BA0AX', fromSsid: 5, type: 'CONTROL', action: 'NORMAL', timeSlot: 29334000, counter: 1, secret: 'ABCDEFGHJKLM' },
  { fromCall: 'BA0AX', fromSsid: 0, type: 'CONTROL', action: 'REBOOT', timeSlot: 29334060, counter: 0, secret: 'ABCDEFGHJKLM' },
  { fromCall: 'BY4SDL', fromSsid: 3, type: 'CONTROL', action: 'NORMAL', timeSlot: 29334000, counter: 7, secret: 'ZYXWVUTSRQPN' }
]
...
describe('calcSignature · FmoLogs baseline fidelity', () => {
  for (const [i, c] of (baseline as BaselineCase[]).entries()) {
    it(`case ${i + 1}: ${c.fromCall}-${c.fromSsid} ${c.action} @${c.timeSlot}/${c.counter}`, () => {
      const sig = calcSignature(c.fromCall, c.fromSsid, c.type, c.action, c.timeSlot, c.counter, c.secret)
...
describe('calcSignature · 形态', () => {
  ...
  it('改变任一输入参数签名应不同', () => {
    ...
    expect(calcSignature('BA0AX', 6, 'CONTROL', 'NORMAL', 29334000, 0, 'ABCDEFGHJKLM')).not.toBe(base)
    expect(calcSignature('BA0AX', 5, 'CONTROL', 'REBOOT', 29334000, 0, 'ABCDEFGHJKLM')).not.toBe(base)
    expect(calcSignature('BA0AX', 5, 'CONTROL', 'NORMAL', 29334001, 0, 'ABCDEFGHJKLM')).not.toBe(base)
    expect(calcSignature('BA0AX', 5, 'CONTROL', 'NORMAL', 29334000, 1, 'ABCDEFGHJKLM')).not.toBe(base)
    expect(calcSignature('BA0AX', 5, 'CONTROL', 'NORMAL', 29334000, 0, 'ZZZZZZZZZZZZ')).not.toBe(base)
  })
})
...
describe('AprsCounter', () => {
  ...
  it('写入后 storage 含 time_slot + counter + last_updated', () => {
    ...
    expect((parsed as { last_updated: string }).last_updated).toMatch(
      /^\d{4}-\d{2}-\d{2}T/
    )
  })
  ...
  it('兼容 FmoLogs 旧数据（无 last_updated 字段）', () => {
    storage.setItem(
      'fmo_aprs_counter',
      JSON.stringify({ time_slot: 29334000, counter: 5 })
    )
...
describe('buildAprsPacket', () => {
  ...
  it('生成符合 APRS 格式的数据包', () => {
    ...
    expect(packet).toMatch(/^BA0AX-5>APFMO0,TCPIP\*::BY4SDL   :CONTROL,NORMAL,29334000,0,[0-9A-F]{16}$/)
  })
...
describe('parseAdif · UTF-8 字节长度（中文）', () => {
  ...
  it('混合中英文字段', () => {
    ...
    expect(parseAdif(input).records).toEqual([
      { call: 'BA0AX', comment: 'BA0AX 你好' }
    ])
  })
