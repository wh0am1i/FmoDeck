...
- **地图满屏铺底**:高德瓦片,深色主题自动反色为"夜图",缩放/平移约束(minZoom 4 + 世界边界),讲话者切换自动 flyTo 定位
- **连线 + 距离标签**:我 ↔ 对方白边青色虚线,中点显示大圆距离(如 `1024 km`)
- **右侧 HUD 信息列**:UTC/本地双时钟横排、☰ 页面菜单(首页隐藏导航栏后的页面切换入口)、放大版讲话者 Hero、讲话名册,纵向一列
- **Hero 集成**:群组名显示与切换(prev/next/搜索)、迷你波形示波器、收听控制(STANDBY 空闲态也能直接开收听)
- **讲话名册升级**:按呼号去重 + ×次数徽标,宽松列表形态(每人一整行),点击在地图上聚焦该呼号 —— 再点取消,新讲话者开口自动解除
- **手机横屏适配**:竖屏全屏提示旋转
- **首页强制深色**:进入首页自动切深色主题(HUD 形态本体),离开还原
- **日志实时刷新**:qso/history 事件触发增量拉取,今日已联 ⭐ / 统计不再停留在连接时刻快照

**改进**

- 讲话者无网格时地图保持当前视角,不再乱跳
- LIVE 态 Hero 边框呼吸辉光

**移除**

- 首页旧三段式布局(顶条波形行 / 独立地图面板 / 底部名册行)整体废弃,波形并入 Hero

### v0.1.9 (2026-05-03)

**新增**

- **SSTV 新模式**:Scottie S1 / Scottie S2
- **超时残图保留**:帧未解完不再直接丢弃,保留已解出的部分行 + 右上角挂"未完整 N/M"徽章
- **手动模式选择**:VIS 漏识别时的救场入口,可强制按指定模式从最近 N 秒开始解码
- **信号指示器**:等待期三色能量条(1900 LEAD / 1200 SYNC / 1700 IMG),直观判断有无 SSTV 信号
- **PNG 元数据**:导出图片写入 mode 信息(SSTVMode / SSTVDisplay / Software tEXt chunk)
- **消息方向区分**:连接后自动获取本机呼号,消息列表 / 详情按 from/to 区分进出方向

**改进**

- **Hampel filter 替换固定中位数**:消除所有 mode 行间"5-行台阶"现象,真实 slant 缓变可平滑跟踪
- **静音超时改时间阈值**(5 秒,所有 mode 统一);长行 mode(PD120)不再被网络抖动误判
- **FM 解调瞬态丢弃**:行起始 5ms warmup 前缀,sync 检测不再被 LPF 启动瞬态污染
- **VIS 检测放宽**:仅需任一段 leader 通过即可识别,弱信号 / QSB 不再漏帧
- **历史筛选**改为动态从模式注册表生成,新增 mode 自动出现
- **SSTV 全文案 i18n** 中英双语
- **消息已读状态跨 nav 保留**:刷新合并时不再覆盖客户端已设置的已读 / to 字段
- **顶部模式列表**改成 chip 风格

**修复**

- 网格坐标点击跳转改为自家项目 `https://maidenmap.wh0am1i.com/`

**已知问题**

- 安卓端接收 SSTV 会出现行间错位,排查中
