...
- 默认不再进入全屏值守屏:打开 app 落地日志页(旧行为),值守屏移到独立路由,导航栏新增「值守」tab 按需进入
...
- 地图满屏铺底(深色夜图),讲话者切换自动定位,我 ↔ 对方连线 + 距离标签
- 右侧信息列:UTC/本地双时钟、☰ 页面菜单、讲话者 Hero、讲话名册
- Hero 显示群组名并可切换群组,内嵌波形与收听控制
- 名册按呼号去重(×次数),点击在地图上聚焦该呼号
- 手机横屏适配,竖屏提示旋转
- 新 QSO 入库实时刷新日志(今日已联 ⭐ / 统计不再滞后)

**修复**

- 自己讲话被误判为对方:本机呼号改用服务器返回值判定(地图连线 / 本机徽章 / 自言自语静音三处)
- iOS 横屏页面可滚、内容被裁(`100vh→100dvh` + 大字体模式限高屏生效)
- 地图可无限缩小到灰板(限制 minZoom + 世界边界)
- 讲话者无网格时地图视角乱跳
- 连线在浅色路网上看不清(白边加粗)
